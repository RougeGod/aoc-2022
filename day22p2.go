...
func main() {
	...
	for _, comm := range commands {
		pos, facing = move(pos, comm.dist, facing)
		if comm.turnRight {
			facing++
		} else {
			facing += 3
...
func move(pos coords, dist int, facing int) (coords, int) {
	for count := 0; count < dist; count++ {
		newPos, newFacing := getNewPos(pos, facing)
		if walls[newPos] {
			return pos, facing
		} else {
			pos = newPos
		...
		if (newPos.x < 50 && newPos.y < 100) || (newPos.x >= 50 && newPos.y >= 150) || (newPos.x >= 100 && newPos.y >= 50) {
			panic("Nonexistent Location")
		}
	}
	return pos, facing
}

func getNewPos(pos coords, facing int) (coords, int) {
	switch facing {
	case 0:
		if pos.x == rowEnds[pos.y][1] {
			switch {
			case pos.y < 50:
				pos = coords{99, 149 - pos.y}
				facing = 2
			case pos.y >= 50 && pos.y < 100:
				pos = coords{50 + pos.y, 49}
				facing = 3
			case pos.y >= 100 && pos.y < 150:
				pos = coords{149, 149 - pos.y}
				facing = 2
			default:
				pos = coords{pos.y - 100, 149}
				facing = 3
			}
		} else {
			pos.x++
	...
	case 1:
		if pos.y == colEnds[pos.x][1] {
			switch {
			case pos.x < 50:
				pos = coords{100 + pos.x, 0}
				facing = 1
			case pos.x >= 50 && pos.x < 100:
				pos = coords{49, 100 + pos.x}
				facing = 2
			default:
				pos = coords{99, pos.x - 50}
				facing = 2
			}
		} else {
			pos.y++
	...
	case 2:
		if pos.x == rowEnds[pos.y][0] {
			switch {
			case pos.y < 50:
				pos = coords{0, 149 - pos.y}
				facing = 0
			case pos.y >= 50 && pos.y < 100:
				pos = coords{pos.y - 50, 100}
				facing = 1
			case pos.y >= 100 && pos.y < 150:
				pos = coords{50, 149 - pos.y}
				facing = 0
			default:
				pos = coords{pos.y - 100, 0}
				facing = 1
			}
		} else {
			pos.x--
	...
	case 3:
		if pos.y == colEnds[pos.x][0] {
			switch {
			case pos.x < 50:
				pos = coords{50, 50 + pos.x}
				facing = 0
			case pos.x >= 50 && pos.x < 100:
				pos = coords{0, 100 + pos.x}
				facing = 0
			default:
				pos = coords{pos.x - 100, 199}
				facing = 3
			}
		} else {
			pos.y--
